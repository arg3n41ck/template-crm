...
## Template policy (overrides all reference workflows)

Required project skill, not an automatic daemon. Scope input to application source and relevant documentation; exclude secrets, `.env*`, dependencies, build output, `.git`, `.ai`, adapter directories and generated graph output. Never upload source merely because an API key exists: remote/paid extraction requires explicit authorization. Use deterministic code extraction by default; semantic extraction may run inline in the current agent, with inferred edges clearly labeled. No specific provider or subagent capability is required.

Read existing graph metadata and compare current source before trusting cached results. Update affected scope after structural changes, not every prompt. `graphify-out/` is disposable local output. Summarize technical facts into `.codex-harness/AGENT_GRAPH.md`; human-reviewed business conclusions belong in `.wiki/`. Do not generate another wiki/Obsidian vault, install hooks, start watch/MCP servers or publish databases unless requested. No edits to AGENTS/CLAUDE from generic reference commands.
...
### Step 1 - Verify an isolated Graphify runtime

Check `graphify --help` and the chosen Python interpreter's `import graphify`. If unavailable, report the prerequisite and ask before installing a reviewed/pinned `graphifyy` release in a project-local virtual environment. Do not use global pip, `--break-system-packages`, automatic upgrades, shell shebang evaluation or installation hooks. Check the installed version's help before using reference commands; package availability is not proof of compatibility.

After verification, create `graphify-out/` and record the verified interpreter path there for the extraction steps below. Treat recorded paths as untrusted data: validate and quote them, never execute a file's contents as a shell command. Keep interpreter paths and generated output out of Git. If runtime/network is unavailable, report Graphify as pending and use current source or available MCP; never label that fallback a Graphify result.
