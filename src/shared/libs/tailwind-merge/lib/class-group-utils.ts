...
export const createClassGroupUtils = (config: AnyConfig) => {
  ...
  const getClassGroupId = (className: string) => {
    const classParts = className.split(classNameSeparator)
...
const getGroupRecursive = (
  ...
  const classRest = classParts.join(classNameSeparator)
...
const getPart = (classPartObject: ClassPartObject, path: string) => {
  ...
  path.split(classNameSeparator).forEach((pathPart) => {
...
): func is ThemeGetter => (func as ThemeGetter).isThemeGetter
